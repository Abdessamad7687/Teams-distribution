...
export default function PostUpdateForm(props) {
    ...
    return (
        <form className="w-100 px-5">
            <h1 className="mt-5">OK "{props.post.title}".</h1>

            <div className="mt-5">
                <label className="h3 form-label">Titre</label>
                <input value={formData.title} name="title" type="text" className="form-control" onChange={handleChange} />
            </div>

            <div className="mt-4">
                <label className="h3 form-label">Dévloppeur</label>
            ...
            <button onClick={() => props.onPostUpdated(null)} className="btn btn-secondary btn-lg w-100 mt-3">Revenir</button>
        </form>
    );
}
